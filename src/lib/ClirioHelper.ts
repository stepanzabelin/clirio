...
export class ClirioHelper {
  ...
  constructor({ modules, scoped }: { modules: Constructor<any>[]; scoped: Scoped }) {
  ...
  public describeModule(module: Constructor<any>): ModuleData[] {
    ...
    for (const [propertyKey, actionData] of actionMap) {
      ...
      const description =
        descriptionTargetMetadata.getDataField(module.prototype, propertyKey, 'description') ?? ''

      const hidden = hiddenTargetMetadata.getDataField(module.prototype, propertyKey, 'hidden')
      ...
      const command = [moduleData.command, actionData.command].filter((f) => f).join(' ')
  ...
  private describeActionOptions(module: Constructor<any>, propertyKey: string): OptionsData[] {
    const optionDescription: OptionsData[] = []

    const optionsArgMap = optionsArgMetadata.getArgMap(module.prototype, propertyKey)
    ...
    if (optionsArgMap.size > 0) {
      ...
      for (const [property, { keys }] of optionsList) {
        ...
        const options = (keys ?? [property]).map((key) => (key.length > 1 ? `--${key}` : `-${key}`))
  ...
  public describeAllModules(): ModuleData[] {
    return this.modules.map((module) => this.describeModule(module)).flatMap((f) => f)
  }
  ...
  private static formatType(type: string | null, itemType: string | null): string {
    if (!type) {
      return ''
    } else if (['number', 'string'].includes(type)) {
      return `<${type}>`
    } else if (type === 'array') {
      const subType = itemType && ['number', 'string'].includes(itemType) ? itemType : ''
      return `<...${subType}>`
    }
    return ''
  }
  ...
  public static formatActionDescription(
    ...
    for (const { options, type, itemType, description } of optionDescription) {
      const col1 = options.join(', ') + ' ' + (showType ? this.formatType(type, itemType) : '')
      content +=
  ...
  public static formatTwoCols(
    ...
    line += col1
    line += col1.length > firstColLen && col2 ? '\n' + ' '.repeat(firstColLen + indent) : ''
